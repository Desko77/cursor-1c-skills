...
Отдельный класс - **справочники API**. Агент не подбирает имя метода по памяти, а берет его из
справочника и проверяет вызов до того, как напишет строку кода. `1c-bsp-api` покрывает Библиотеку
стандартных подсистем: 2624 метода в 284 модулях, версии 3.1.11 и 3.2.1, с сигнатурами, типами
параметров, контекстами выполнения и 372 переопределяемыми обработчиками; команда `check` отвергает
вызов, которого в библиотеке нет. `zup-hr-api-reference` - 1С:ЗУП 3.1: кадровый учет, средний
заработок и начисления, пособия и обмен с СФР, регламентированная отчетность.
