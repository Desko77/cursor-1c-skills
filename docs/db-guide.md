...
| `/db-dump-dt` | `.ps1` | Выгрузка всей ИБ в DT-файл (полный бэкап: данные + конфигурация) |
| `/db-load-dt` | `.ps1` | Восстановление ИБ из DT-файла (деструктивная: полная замена базы) |
...
2. Если указал **базу по имени** - ищи: `id` → `aliases` (с учетом морфологии) → `name` (нечеткое)
...
```
> Создай файловую базу C:\Bases\Test и загрузи в нее конфигурацию из C:\WS\cfsrc
```
...
```
> Выгрузи расширение МоеРасширение из базы dev в C:\WS\ext_src
```

Claude вызовет `/db-dump-xml dev C:\WS\ext_src -Extension МоеРасширение`.

```
> Загрузи расширение обратно в базу
```

Claude вызовет `/db-load-xml C:\WS\ext_src dev -Extension МоеРасширение` → предложит `/db-update -Extension МоеРасширение`.

### Полный бэкап и восстановление базы

```
> Сделай полный бэкап базы dev в C:\backup\dev.dt
```

Claude вызовет `/db-dump-dt dev C:\backup\dev.dt` (в DT попадает вся база: данные, конфигурация, пользователи).

```
> Восстанови тестовую базу из C:\backup\dev.dt
```

Claude предупредит о полной замене содержимого базы, запросит подтверждение и вызовет `/db-load-dt C:\backup\dev.dt test`. `/db-update` после восстановления не нужен.
