...
3. Объединяет границы всех таблиц → базовая решетка
...
Claude вызовет `/mxl-info` и выведет области, параметры и наборы колонок. С флагом `-WithText` покажет еще и текстовое содержимое ячеек (заголовки колонок, надписи).
...
3. `/mxl-compile` → обновленный Template.xml
