...
1. **В проект** (рекомендуется) - скачать essentials build с https://www.gyan.dev/ffmpeg/builds/, распаковать в `tools/ffmpeg/`. Код найдет `tools/ffmpeg/bin/ffmpeg.exe` автоматически
...
await showCaption('Создаем новый заказ');
await wait(1.5);
...
Ритм "субтитр → пауза → действие" дает зрителю время прочитать, что произойдет:
...
| `ru-RU-SvetlanaNeural` | Женский, русский - четкий, уверенный |
...
| Видео дергается | Добавьте `wait()` между шагами |
...
- [Тестирование через веб-клиент](web-test-guide.md) - навигация, формы, таблицы, отчеты
