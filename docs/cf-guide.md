...
|-------|-----------|----------|
| `/cf-info` | `<ConfigPath> [-Mode overview\|brief\|full]` | Анализ конфигурации: свойства, состав, счетчики объектов (3 режима) |
...
1. `/cf-init` создает scaffold пустой конфигурации (Configuration.xml, ConfigDumpInfo.xml, Languages/)
...
### overview (по умолчанию) - заголовок + ключевые свойства + счетчики
...
Выводит все свойства по категориям (скалярные, enum, ref), полный список ChildObjects поименно, DefaultRoles и мобильные функциональности.
