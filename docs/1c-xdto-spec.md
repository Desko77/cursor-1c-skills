...
```
XDTOPackages/
├── ОбменСБанком.xml              # Объект метаданных - 4 свойства, и все
├── ОбменСБанком/
│   └── Ext/
...
| `Comment` | Комментарий |
| `Namespace` | URI целевого пространства имен. Дублирует `targetNamespace` в `Package.bin` |
...
Всего восемь имен элементов. `typeDef` - анонимный (встроенный) тип; его разновидность
задает `xsi:type` = `ValueType` либо `ObjectType`.
...
|---|---|---|
| `targetNamespace` | URI | Целевое пространство имен |
...
## 4. Схема префиксов пространств имен

Каждая ссылка на тип из пространства имен, отличного от `xs:`/`xsi:`, требует объявления
префикса вида `dNpM`:
...
- `M` - порядковый номер нового пространства имен, объявляемого на этом узле (с 1).
...
Целевое пространство имен самого пакета исключением не является - ссылка на собственный
...
пространство имен все равно проходит через общий механизм выделения префиксов. Если оно уже
...
префиксом пространства имен модели XDTO (хотя по умолчанию оно и так объявлено как `xmlns=`):
...
При импорте XML-схемы через Конфигуратор тип из чужого пространства имен, для которого
...
При этом `<import namespace="..."/>` в пакете остается. Признак "объявлен импорт, но
ни один тип из этого пространства имен не используется" - надежный индикатор проблемы.
