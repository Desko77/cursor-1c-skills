...
- Каталог `<Имя>/` создается только если есть вложенные подсистемы или файл `Ext/CommandInterface.xml`
...
| `Content` | Content | да | Список включенных объектов метаданных. Пустой тег `<Content/>` если нет |
...
Список объектов метаданных, включенных в подсистему:
...
<Content>
    ...
    <xr:Item xsi:type="xr:MDObjectRef">Report.Отчет</xr:Item>
</Content>
<ChildObjects>
    <Subsystem>Дочерняя1</Subsystem>
</ChildObjects>
...
```xml
<CommandsOrder>
    <Command name="Document.СчетНаОплату.StandardCommand.Create">
...
- `ActionsPanelReports` - отчеты в панели действий
...
> **Порядок свойств**: В командах объектов `Representation` идет **после** `ModifiesData`, а в CommonCommand - после `Comment`. Это единственное структурное различие.
...
| `Open` | Открыть (для отчетов, обработок, общих форм) |
...
Ссылаются на объекты метаданных `CommandGroup`, определенные в конфигурации (каталог `CommandGroups/`).
