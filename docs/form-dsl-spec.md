...
### Дополнения командной панели динамического списка

Три дополнения таблицы - строка поиска, состояние просмотра, управление поиском - живут в
двух разных позициях, и это разные вещи.

**Штатное дополнение таблицы.** Без настройки компилятор выдает пустой тег, платформа
дополняет его сама. Отличия от умолчания задаются ключом `additions` у таблицы, и тогда
дополнение пишется целиком:
...
**Объявленное дополнение.** Отдельный элемент командной панели со своим именем. Задается
тип-ключом `searchString`, `viewStatus` или `searchControl` внутри `commandBar` таблицы:
...
Объявленное дополнение НЕ заменяет штатное: платформа держит оба. Замер на 8.3.27.2214.

| Ключ | Умолчание | Пояснение |
|---|---|---|
| `source` | имя родительской таблицы | элемент источника дополнения |
| `title`, `visible`, `width`, `horizontalStretch` | - | как у поля |
| `horizontalLocation` | `auto` | `left`, `right`, `center` и русские синонимы; умолчание в XML не пишется |

Тип источника задается видом дополнения и в DSL не указывается:
`SearchStringRepresentation`, `ViewStatusRepresentation`, `SearchControl`.

Порядок внутри дополнения снят с выгрузки платформы: источник, свойства, контекстное меню,
расширенная подсказка.

В редакторе формы (`1c-form-edit`) те же тип-ключи доступны, но `source` там обязателен:
редактор дописывает элемент в готовую форму и родительскую таблицу по входу не знает.
