...
Размещается в корне проекта (рядом с `.git/`). Создается навыком `/db-list add` или вручную.
...
2. Если указал **базу по имени** - поиск: `id` → `aliases` (с учетом морфологии) → `name` (нечеткое)
...
URL для открытия базы в браузере через `/web-test`. Задается в записи конкретной базы.

Если не задан - `/web-test` берет URL из активной веб-публикации (`/web-publish`).
