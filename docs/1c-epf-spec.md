...
> **Связанная спецификация**: Для внешних отчетов (ExternalReport / ERF) см. [1c-erf-spec.md](1c-erf-spec.md). Формат отчетов основан на формате обработок с дополнительными свойствами для СКД и вариантов.
...
Корневой элемент - `<MetaDataObject>`, пространство имен:
...
Корневой элемент - `<Form>`, пространство имен:
...
|---|---|---|---|
| Табличная часть объекта | `Объект.<ИмяТЧ>` | `Объект.<ИмяТЧ>.<ИмяРеквизита>` | (определен в корневом XML) |
...
> **ВАЖНО**: Для коллекций (ValueTable, ValueTree) тип **обязателен**. Пустой `<Type/>` приведет к ошибке "Неверный путь к данным" при обращении к колонкам через DataPath.
...
**Проблема**: При загрузке формы конфигуратор выдает ошибку "Неверный путь к данным: <Реквизит>.<Колонка>" для всех колонок таблицы.
...
4. Для каждой формы:
   ...
   - Создать `Form.xml` (раздел 6) - проверить пространство имен!
