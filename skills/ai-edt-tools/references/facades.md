...
У `text_search` `projectName` НЕОБЯЗАТЕЛЕН: без него ищутся все открытые проекты с исходниками, а ответ называет их поимённо. Это важно, когда рядом лежат конфигурация и её расширения - пустой ответ по одному проекту читается как «такого текста нет вообще». Явно названный несуществующий проект остаётся ошибкой.
...
`checkId`. Это не «починить всё»: исправление есть не у каждой проверки.
