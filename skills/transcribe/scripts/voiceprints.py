...
Слой "по голосу" (абсолютная идентификация, узнает даже неназванных, помнит между встречами) -
...
#   уверенно различимых, похожих/неоднозначных отдаем text-binding'у (слои дополняют друг друга).
    ...
    if args.cmd == "enroll":
        ...
        for spk, name in mp.items():
            if spk in prints:
                ...
                print(f"  занесен {spk} -> {name.strip()}")
            else:
