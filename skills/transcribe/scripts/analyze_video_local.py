...
прерывает разбор: кадр помечается, прогон идет дальше. Что сделано, а что нет, пишется в
...
#   разбор совсем и бросало весь хвост кадров без единой попытки; теперь по этому порогу идет
...
def run_whisper(video: Path, output_dir: Path, diarize=False, extra=None, reuse=False):
    ...
    if not Path(WHISPER_PYTHON).exists():
        raise lb.LocalBackendError(
            ...
            "Задай env WHISPER_PYTHON с корректным путем.")
...
def parse_transcript(output_dir: Path, base: str):
    """Разобрать транскрипцию -> [(start_sec, text)]. При наличии диаризации берет файл со спикерами
...
def analyze_frames(video: Path, output_dir: Path, segs, detailed_path: Path, status_path: Path,
    ...
    Сбой кадра больше не прерывает стадию: неудачный помечается маркером, разбор идет дальше.
    ...
    if reuse:   # резюм: описания прошлого прогона берем готовыми, заново зрение не гоняем
    ...
    # Первый кадр идем ОДИН: его prompt_tokens и есть настоящий резерв контекста под картинку
...
def main():
    ...
    ap.add_argument("--no-vlm", action="store_true",
                    help="Не разбирать экран моделью зрения (речь, спикеры и саммари считаются как обычно). "
                         "Кадры при этом все равно нарезаются - их можно посмотреть глазами")
    ...
    def speaker_llm(data, instruction):
        ...
        послушание модели. Отдаем текстом - text_stage разбирает ответ сам и одинаково понимает
    ...
    # ----- Предполетная проверка: постадийно, а не все-или-ничего -----
    ...
    if args.no_vlm:
        print("[0] разбор экрана отключен (--no-vlm)", flush=True)
    ...
    # обнуляла и именование, и авто-enroll голосовой базы. Свопа моделей перенос не создает -
    # на 150 они со-резидентны.
    transcript_text = _transcript_text(segs)

    # СЛОЙ 1 - голос (база отпечатков, cosine > порог): узнает различимых даже неназванных и между
    # встречами. СЛОЙ 2 - текст (модель на 150). СЛОЙ 3 - программная проверка по обращениям
    # (speaker_validator): без нее модель систематически вешает имя на того, кто его ПРОИЗНОСИТ,
    ...
    if transcript_text.strip():
        ...
        if use_speaker_llm:
            ...
            # текстового слоя прошел бы молча.
        ...
        if use_voice and db is not None and name_map:   # авто-enroll: голос не узнал, но имя есть
            ...
            # подсчет неоднозначностей больше не нужен: в базу не попадут голоса разных людей
            # под одной записью.
            added, skipped = 0, 0
            ...
            if added:
                try:
                    ...
                    print(f"[спикеры] авто-enroll не сохранен ({e})", file=sys.stderr)
            elif skipped:
                print(f"[спикеры] авто-enroll пропущен: все {skipped} имен неоднозначны", flush=True)
    ...
    if use_vlm:
        ...
        # Зрение выключено - кадры все равно нарезаем: их можно посмотреть глазами, а речь по
        ...
        try:
            ...
            # Кадры перечисляем поименно даже без разбора: по этому списку человек или агент
