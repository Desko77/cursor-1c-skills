...
def main():
    ...
    if args.command == 'check':
        ...
        if hit:
            ...
            # Существует - еще не значит "вызывай": устаревший метод по виду
            # не отличается от действующего. Код возврата не меняем, метод
            # существует; предупреждение читает человек либо валидатор.
            dep = hit[0].get('dep')
            if dep:
                print('  ВНИМАНИЕ: устаревший, вместо него %s' % dep)
            return 0
