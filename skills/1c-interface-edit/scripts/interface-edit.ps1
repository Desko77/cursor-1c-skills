...
# Гард до любой записи: -CreateIfMissing иначе успеет создать файл в конфигурации на замке.
...
# --- Auto-validate ---
if (-not $NoValidate) {
	$validateScript = Get-SiblingSkillScript "interface-validate" "interface-validate.ps1"
	if ($validateScript) {
		Write-Host ""
