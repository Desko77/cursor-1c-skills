...
Создаёт проектный `CLAUDE.md` в корне проекта - авто-загружаемый каждую сессию контекст для агента:
...
- **Этот скил** - опинионейтед агент-контекст: правила-грабли как IF-THEN, протокол, primacy/recency,
 "док как часть работы". Используй, когда проект сложный и нужен надёжный контекст для агента.
...
- **ФОРМАТ соблюдён** по глобальным правилам (`~/.claude/rules/text-formatting.md`, `~/.claude/CLAUDE.md`):
 напр. без `ё` (заменить на `е`), без длинных тире (`-` вместо `—`/`–`). Проверка: `grep -nE 'ё|—|–' CLAUDE.md`.
...
- ${ещё критичные инварианты проекта}
