...
description: "Справочник паттернов компоновки управляемых форм 1С. Используй как справочник при проектировании форм - архетипы, конвенции, продвинутые приемы"
---
...
{ "cmdBar": "КоманднаяПанель", "children": [
 ...
 { "button": "ПечатьСчет", "command": "ПечатьСчет" }
 ]}
]}
```
...
Вместо кнопки для открытия подформ (ЦеныИВалюта, УчетнаяПолитика):
