...
```powershell
powershell.exe -NoProfile -File skills/1c-db-dump-cf/scripts/db-dump-cf.ps1 <параметры>
```
...
| `-AllExtensions` | нет | Выгрузить все расширения (платформа 8.3.20+, на старых версиях используйте `-Extension <имя>` отдельно для каждого расширения) |
...
powershell.exe -NoProfile -File skills/1c-db-dump-cf/scripts/db-dump-cf.ps1 -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -OutputFile "C:\backup\config.cf"
