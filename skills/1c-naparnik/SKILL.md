...
- Альтернатива `syntaxcheck` (1c-syntax-checker-mcp) с расширенным анализом логики
...
2. ПРОВЕРИТЬ каждое замечание по факту: существование метода и сигнатуру - через bsl-platform-help или
 исходники БСП, поведение - через код проекта. Неподтвержденное замечание НЕ исправлять
3. rewrite_1c_code(code="<код>", goal="производительность") - вариант ИИ
 ИЛИ
 modify_1c_code(instruction="вынести запрос в отдельную функцию", code="<код>") - точечное изменение
4. Результат генерации ИИ тоже проверить: вызванные методы должны существовать
5. ask_1c_ai(question="<новый код>") - проверка результата
```

> Напарник дает false positives и ссылается на несуществующие API. Правка кода по непроверенному замечанию
> или прием сгенерированного варианта без сверки методов - источник багов. Обязательный порядок -
> `rules/mcp-tool-priority.md`, раздел "Обязательные проверки".
...
|--------|-------------|-------------|-----------|
| Синтаксис BSL (изолированный код) | `check_1c_code` | `syntaxcheck` (1c-syntax-checker-mcp) | Напарник дополнительно проверяет логику и производительность |
| Синтаксис BSL (в контексте проекта EDT) | `check_1c_code` | `get_project_errors` (ai-edt) | EDT проверяет в контексте проекта (типы, ссылки), Напарник - изолированный код |
| Валидация запросов | - | `validate_query` (ai-edt) | EDT для запросов (семантика + контекст метаданных), Напарник не валидирует запросы отдельно |
...
| Функции БСП | - | `ssl_search` (1c-mcp_ssl_server) | Только в БСП-сервере |
...
| Функция/модуль БСП | `ssl_search` | 1c-mcp_ssl_server |
| Код в проекте (полнотекстовый/regex) | `search_in_code` | ai-edt |
| Структура объекта метаданных | `get_metadata_details` | ai-edt |
