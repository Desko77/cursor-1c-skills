...
try {
    ...
    $resultFile = Join-Path $tempDir "dump_dt_result.txt"
    ...
    $exitCode = Write-PlatformVerdict -ExitCode $exitCode -ResultFile $resultFile -LogText $logContent `
        ...
        -SuccessMessage "Infobase dumped successfully to: $OutputFile" `
        -FailureMessage "Error dumping infobase. Check for active sessions - dump needs exclusive access." `
        -Strict:$StrictLog
