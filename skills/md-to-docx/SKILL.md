...
**Есть образец документа?** Тогда этот скил не подходит: он делает собственное оформление.
Когда документ должен выглядеть как присланный образец (шаблон заказчика, предыдущий документ
серии, эталон отдела) - брать скил `docx-from-sample`: он наследует стили, нумерацию заголовков,
оформление таблиц и колонтитулы прямо из файла-образца.
...
| `--no-shading` | Нет | Отключает серый фон у inline-кода (`код`) и у блоков ``` code ``` . Эквивалент: `--shading=off`. По умолчанию фон включен. На шапку таблиц не влияет - она остается со структурным фоном |
