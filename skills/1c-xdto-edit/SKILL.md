---
name: 1c-xdto-edit
description: "Точечное редактирование пакета XDTO 1С. Используй когда нужно добавить, изменить или удалить тип или свойство в существующем пакете, переименовать пакет, сменить пространство имен"
---
...
| `NoValidate` | нет | Не запускать `1c-xdto-validate` после правки |
...
| `add-import` | - | URI пространства имен |
| `rename` | - | новое имя объекта метаданных |
| `set-synonym` | - | синоним |
| `set-comment` | - | комментарий |
| `set-namespace` | - | новый URI пространства имен |

Батч через `;;` там, где перечисление осмысленно: `remove-property`, `remove-type`,
`add-enum`, `add-import`.

```powershell
... -Operation add-property -Target "Платеж" -Value '<xs:element name="Комментарий" type="xs:string" minOccurs="0"/>'
... -Operation remove-property -Target "Платеж.Комментарий ;; Платеж.Черновик"
...
надежен только для однострочных фрагментов без вложенных кавычек.
...
и `<Namespace>` объекта метаданных. Пакеты, импортирующие старое пространство имен,
