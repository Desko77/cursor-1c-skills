...
Типичный сценарий: пользователь видит колонку "Коэффициент Ки" в отчете и спрашивает как она считается. Один вызов `trace` показывает: формулу вычисления, откуда берутся операнды, как агрегируется в ресурс.

## Что не выводится

- XML namespace-декларации
- Обертки v8:item/v8:lang/v8:content (извлекаем чистый текст)
