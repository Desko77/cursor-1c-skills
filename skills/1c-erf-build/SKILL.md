...
```powershell
powershell.exe -NoProfile -File .claude/skills/1c-epf-build/scripts/epf-build.ps1 <параметры>
```
...
powershell.exe -NoProfile -File .claude/skills/1c-epf-build/scripts/epf-build.ps1 -InfoBasePath "C:\Bases\MyDB" -SourceFile "src/МойОтчёт.xml" -OutputFile "build/МойОтчёт.erf"

# Серверная база
powershell.exe -NoProfile -File .claude/skills/1c-epf-build/scripts/epf-build.ps1 -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -SourceFile "src/МойОтчёт.xml" -OutputFile "build/МойОтчёт.erf"
```
