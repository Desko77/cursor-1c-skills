...
дважды, без скила и со скилом, и ответы сравниваются с ожидаемыми и запрещенными строками. Ключ
`--routing` проверяет другое - выбор скила: раскладывает несколько скилов сразу и определяет, чьи
файлы агент открыл. Ключ `--resume` продолжает прерванный заход, не пересчитывая сделанное.

```bash
python tools/run_skill_evals.py --skill 1c-bsp-api --runs 3
