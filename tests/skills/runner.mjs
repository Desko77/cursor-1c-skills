...
const SKILLS    = resolve(REPO_ROOT, 'skills');
const CASES     = resolve(ROOT, 'cases');
const CACHE     = resolve(ROOT, '.cache');

// ─── Раскладка скилов ───────────────────────────────────────────────────────
// Кейсы адресуют скрипты короткими именами (`meta-compile/scripts/meta-compile`), а в этом
// репозитории 1С-скилы лежат с префиксом `1c-`. Правим не 849 кейсов, а одно место резолва:
// так кейсы остаются сравнимыми с исходным набором и переносимы обратно.
// Здесь перечислены только те скилы, у которых имя расходится сильнее префикса.
const SKILL_ALIASES = {
  'epf-init':     '1c-epf-scaffold',
  'help-add':     '1c-help-manage',
  'support-edit': '1c-support-state',
};
// Отдельные случаи, где расходится и ИМЯ ФАЙЛА скрипта, а не только каталог скила.
const SCRIPT_ALIASES = {
  'support-edit/scripts/support-edit': '1c-support-state/scripts/support-state',
};

// Каталог скила по короткому имени из кейса.
function skillDirCandidates(skill) {
  const out = [];
  if (SKILL_ALIASES[skill]) out.push(SKILL_ALIASES[skill]);
  out.push(`1c-${skill}`, skill);
  return out;
}
...
function ensureSetup(setupName, runtime, skillCasesDir) {
  ...
  const fixtureStamp = (args) => {
    ...
    for (const ext of ['.ps1', '.py']) {
      const p = resolve(SKILLS, '1c-cf-init/scripts/cf-init' + ext);
...
function resolveScript(scriptRelPath, runtime) {
  ...
  const tried = [];
  for (const rel of skillScriptCandidates(scriptRelPath)) {
    const full = join(SKILLS, rel + ext);
    tried.push(full);
    if (existsSync(full)) return full;
  }
  throw new Error(`Script not found: ${tried.join(' | ')}`);
}

// Все варианты пути `<скил>/scripts/<файл>` с учетом нашей раскладки.
function skillScriptCandidates(scriptRelPath) {
  if (SCRIPT_ALIASES[scriptRelPath]) return [SCRIPT_ALIASES[scriptRelPath]];
  const slash = scriptRelPath.indexOf('/');
  if (slash < 0) return [scriptRelPath];
  const skill = scriptRelPath.slice(0, slash);
  const rest = scriptRelPath.slice(slash);
  return skillDirCandidates(skill).map((d) => d + rest);
}
...
async function discoverIntegration(filter) {
  ...
  // Тест, записывающий фикстуру в кэш, идет раньше тех, которые ее читают: имена файлов
  // дают обратный порядок (build-cfe до build-config), и на пустом кэше чтение отказывает.
...
// ─── Известные разрывы ──────────────────────────────────────────────────────
// Часть кейсов красная не из-за регресса, а из-за того, что скрипт этого набора пока не умеет
// того, что умеет исходный (нет типа объекта, операции, параметра). Пока такие кейсы просто
// падают, набор нельзя использовать как гейт: красным он будет всегда, и НОВОЕ падение в нем
// не видно. Реестр известных разрывов разделяет эти два случая.
//
// Файл `known-gaps.json` - список id кейсов. Правило гейта:
//   - упал кейс ИЗ реестра        -> ожидаемо, гейт не роняем;
//   - упал кейс ВНЕ реестра       -> регресс, гейт красный;
...
function applyKnownGaps(results, casesOk, scope) {
  ...
  let known;
  try {
    const parsed = JSON.parse(readFileSync(KNOWN_GAPS_FILE, 'utf8'));
    known = new Set(Array.isArray(parsed) ? parsed : parsed.cases || []);
  } catch (e) {
    console.log(`  known-gaps.json не разобран (${e.message}) - гейт по нему не применяется`);
    return casesOk;
  }
  ...
  if (regressions.length) {
    console.log(`  РЕГРЕСС: ${regressions.length} кейс(ов) упало вне реестра известных разрывов:`);
    for (const r of regressions.slice(0, 20)) console.log(`    - ${r.id}`);
    if (regressions.length > 20) console.log(`    ... и еще ${regressions.length - 20}`);
    return false;
  }
  return true;
}
